...
# ─────────────────────────────────────────
st.set_page_config(
    page_title="Mobile Phone Price Tracker",
    page_icon="📱",
    layout="wide"
)
...
def load_data():
    ...
    # Flag brand-only names (no model info) — append "(Model Unknown)"
    # A name is considered brand-only if it's just one word or matches brand exactly
    def flag_no_model(row):
        name  = row['name'].strip()
        brand = row['brand'].strip().title()
        words = name.split()
        if len(words) <= 1 or name.lower() == brand.lower():
            return f"{name} (Model Unknown)"
        return name

    df['name'] = df.apply(flag_no_model, axis=1)
...
def chart_price_history(result):
    ...
    # Bridge line — connects last historical point to first forecast point
    # fills the gap between last scraped date and today's forecast start
    ...
    # Confidence band — includes bridge gap + forecast period
...
# 6. UI — HEADER
# ─────────────────────────────────────────
st.markdown('<div class="hero-title">📊 Mobile Phone Price Tracker</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════
with tab1:
    st.markdown("### Search for a Phone")
    ...
    if product_summary.empty:
        ...
        if selected_key:
            ...
            # Try to forecast — if data is corrupted or insufficient, show friendly error
            try:
