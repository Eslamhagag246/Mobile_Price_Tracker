...
.hero-title {
    ...
    background: linear-gradient(135deg, #ff6b9d 0%, #7b5cf0 60%, #00d4ff 100%);
...
search = st.text_input("🔎 Search phone model", 
                       placeholder="e.g. iPhone, Galaxy, Xiaomi, Realme...",
                       help="Search for mobile phones by name")
...
# ─────────────────────────────────────────
if filtered.empty:
    st.warning("⚠️ No phones found. Try different filters.")
    st.stop()
...
st.markdown(f"**Found {len(product_summary)} phones**")
...
selected_key = st.selectbox(
    ...
    help="Select a phone to see price forecast"
)
...
c1.markdown(f"""<div class="stat-card">
    ...
    <div class="stat-value" style="color:#ff6b9d">EGP {result['last_price']:,.0f}</div>
